// ? 동기 처리 비동기 처리에서 항상 꼬리표처럼 따라라디는 말이 블로킹과 논블로킹이다.
// ? 쉽게 생각하면 블로킹은 함수 실행 도중 다른 함수의 실행이 발생하면 실행 제어권을 그 다른 함수에게 넘기기때문에 현재 실행중인 함수의 실행이 멈추는것이고
// ? 논블로킹은 다른 함수를 만나서 그 함수가 실행되더라도(그 함수는 비동기 처리 함수 일것이다.) 제어권을 넘기지 않고 자신이 가지는 것이다.
...
// 위 예제의 sleep함수는 3초 후에 foo 함수를 호출한다. 이때 bar 함수는 sleep 함수의 실행이 종료된 이후에 호출되므로 3초이상 호출되지 못하고 블로킹된다.
// * 코드 실행 제어권이 sleep함수에게 있기 때문
