...
- 클라이언트가 리소스 위치를 알고 URI 지정(POST와의 명백한 차이점이다. 클라이언트가 리소스의 전체 경로를 정확히 알고 있음)
- 아래 주석의 Request line을보면 'PUT /members/100 HTTP/1.1' 이렇게 돼있는데 정확이 /members의 /100 이라는 URI전체로 요청을 보낸다.
...
여기서 주의 해야할 점은 PUT은 리소스를 완전히 대체한다는 것이다.
데이터의 일부분을 바꾸고 싶다고 해서
요청 바디부분을
{
  "age": 50
}
이렇게 작성해서 PUT 요청을 보낸다면 URI에 해당하는 리소스가 완전히 대체되서 다른 정보는 전부 사라지고 "age"에 대한 정보만 남게되는 것이다.
일부를 수정하는 요청이 PATCH.(POST로도 할 수는 있다. PATCH를 지원안하는 브라우저가 있을 수 있음. 그치만 기본적으로 PATCH를 사용하자.)
```
...
PATCH를 사용하면 리소스의 부분 변경을 할 수 있다.
```
